...
@attrs.frozen
class Environment:
    """An environment is a frozen copy of a set of files and symlinks.

    The files are usually source code and configuration files while the symlinks link
    output directories like an aim or w&b repository into the environment, so that job
    outputs from different environments go into a common place.
    """

    path: Path

    @staticmethod
    def prepare(config: Config) -> "Environment":
